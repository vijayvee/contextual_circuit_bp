...
def plot_fits(
    ...
    for idx in sorted_perfs:
        ...
        if stimulus_type == 'sparse_noise_8':
            stimuli = os.path.join(
                stimulus_dir,    
                'locally_sparse_noise_8deg_template.pkl')
        elif stimulus_type == 'sparse_noise_4':
            stimuli = os.path.join(
                stimulus_dir,
                'locally_sparse_noise_4deg_template.pkl')
        elif stimulus_type == 'drifting_grating':
            stimuli = os.path.join(
                stimulus_dir,
                'locally_sparse_noise_8deg_template.pkl')
        stimuli_data = pickle.load(open(stimuli, 'rb'))
        import ipdb;ipdb.set_trace()
        with tf.device('/cpu:0'):
            val_images = tf.placeholder(
                tf.float32,
                shape=[None] + dataset_module.model_input_image_size)  
        ...
        try:
            while not coord.should_stop():
                ...
                if step == 0:
                    filters = val_dict['filts'].squeeze()
