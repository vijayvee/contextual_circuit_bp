...
class ContextualCircuit(object):
    ...
    def full(self, i0, O, I):
        ...
        Normalizations:
        A_U = (h_Ot-1 x U_c) * U_k
        A_T = (h_Ot-1 x T_c) * T_k
        A_Q = (h_It-1 x Q_c) * Q_k
        A_P = (h_It-1 x P_c) * P_k

        Gates:
        f_It = sigmoid(X * I_k_input + h_It * I_k_h)
        f_Ot = sigmoid(X * O_k_input + h_Ot * O_k_h)
