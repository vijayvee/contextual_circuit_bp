...
class ContextualCircuit(object):
    ...
    def __init__(
        ...
        # Kernel shapes
        self.SRF, self.SSN, self.SSF, self.SSNF = SRF, SSN, SSF, SSNF
        self.SSN_ext = 2 * py_utils.ifloor(SSN / 2.0) + 1
        self.SSF_ext = 2 * py_utils.ifloor(SSF / 2.0) + 1
        self.SSNF_ext = 2 * py_utils.ifloor(SSNF / 2.0) + 1
        if self.SSN is None:
            self.SSN = self.SRF * 3
        if self.SSF is None:
            self.SSF = self.SRF * 5
        if self.separable:
            ...
            self.pt_shape = [self.SSNF_ext, self.SSNF_ext, 1, 1]
        else:
            ...
            self.pt_shape = [self.SSNF_ext, self.SSNF_ext, self.k, self.k]
        ...
        if self.near_far_club:
            self.tuning_params = ['Q']# , 'T']
            self.pt_shape = [self.SSNF_ext, self.SSNF_ext, self.k, self.k]
        elif self.association_field:
            self.tuning_params = ['Q', 'T']
            self.p_shape = [self.SSN_ext, self.SSN_ext, self.k, self.k]
        else:
    ...
    def prepare_tensors(self):
        ...
        #############################################
        if self.near_far_club:
            pt_array = np.ones(self.pt_shape)
            pt_array = pt_array / pt_array.sum()
            if 'PT' in self.lesions:
                print 'Lesioning near eCRF.'
                pt_array = np.zeros_like(pt_array).astype(np.float32)
            else:
                setattr(
                    self,
                    self.weight_dict['PT']['r']['weight'],
                    self.symmetric_weights(tf.get_variable(
                        name=self.weight_dict['PT']['r']['weight']+'_non_symm',
                        dtype=self.dtype,
                        initializer=initialization.xavier_initializer(
                            shape=self.pt_shape,
                            uniform=self.normal_initializer),
                        trainable=True),self.weight_dict['PT']['r']['weight']))
        else:
        ...
        if 'Q' in self.lesions:
            ...
            setattr(
                ...
                self.weight_dict['Q']['r']['tuning'],
                self.symmetric_weights(tf.get_variable(
                    name=self.weight_dict['Q']['r']['tuning']+'_non_symm',
                    dtype=self.dtype,
                    trainable=True,
                    initializer=initialization.xavier_initializer(
            ...
            if not self.association_field:
                ...
                if 'P' in self.lesions:
                    ...
                    setattr(
                        ...
                        self.weight_dict['P']['r']['tuning'],
                        self.symmetric_weights(tf.get_variable(
                            name=self.weight_dict['P']['r']['tuning']+'_non_symm',
                            dtype=self.dtype,
                            trainable=True,
                            initializer=initialization.xavier_initializer(
                                shape=self.tuning_shape,
                                uniform=self.normal_initializer,
                                mask=None)),self.weight_dict['P']['r']['tuning']))
            if 'T' in self.lesions:
                ...
                setattr(
                    ...
                    self.weight_dict['T']['r']['tuning'],
                    self.symmetric_weights(tf.get_variable(
                        name=self.weight_dict['T']['r']['tuning']+'_non_symm',
                        dtype=self.dtype,
                        trainable=True,
                        initializer=initialization.xavier_initializer(
                            shape=self.tuning_shape,
    ...
    def full(self, i0, O, I):
        ...
        if not self.near_far_club:
            pt_weights = self[
                self.weight_dict['P']['r']['weight']]
            pt_weights = tf.minimum(p_weights,0)  # Force excitatory conns
            g = tf.get_default_graph()
            with g.gradient_override_map({"Conv2D": "SymmetricConv"}):
                T = self.conv_2d_op(
                    data=O,
                    weight_key=self.weight_dict['PT']['r']['weight'],
                    weights=pt_weights
                )
        else:
            T = self.conv_2d_op(
        ...
        if self.near_far_club:
            pt_weights = self[
                self.weight_dict['PT']['r']['weight']]
            pt_weights = tf.nn.relu(p_weights)  # Force excitatory conns
            g = tf.get_default_graph()
            with g.gradient_override_map({"Conv2D": "SymmetricConv"}):
                T = self.conv_2d_op(
                    data=I,
