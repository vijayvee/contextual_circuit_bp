...
while [ $RUN_LOOP == "true" ]
	do
		CHECK="$(psql -U contextual_dcn_p3 -h 127.0.0.1 -d contextual_dcn_p3 -c 'SELECT * from experiments h WHERE NOT EXISTS (SELECT 1 FROM in_process i WHERE h._id = i.experiment_id)')"
